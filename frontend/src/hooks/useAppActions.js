...
export function useAppActions(state) {
  ...
  const addToCart = (product, qty, unit, extra = {}) => {
    ...
    if (extra && extra.is_sample) {
      const len = Number(qty) > 0 ? Number(qty) : 1;
      ...
      setNotice(`${product.name} — sampel ${len} ${product.base_unit} masuk keranjang (permintaan potong ke gudang).`);
      return;
    }
