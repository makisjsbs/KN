...
export function CheckoutItemCard({
  ...
  return (
    <div data-testid={`cart-item-${pid}`} className="rounded-md border border-[#EFF0F2] bg-white p-2.5">
      ...
      <div className="mt-2">
        <label className="text-[9px] font-bold uppercase tracking-wide text-[#8E8E93]">{isSample ? `Panjang sampel (${baseUnit})` : `Qty (${baseUnit})`}</label>
        <input data-testid={`cart-item-qty-input-${pid}`} className="field" type="number" min={isSample ? "0.1" : "1"} step={isSample ? "0.1" : "1"} value={item.quantity} onChange={(e) => onUpdateQty(pid, e.target.value)} />
        <p data-testid={`cart-item-rolls-${pid}`} className="mt-1 text-[10px] text-[#9A9BA3]">{rollCount} roll tersedia · {isSample ? "dipotong gudang dari roll (RFID)" : `dijual per ${baseUnit}`}</p>
      </div>
      {isSample && <SampleLineFields item={item} onUpdateSample={onUpdateSample} />}
