...
import {
  ...
  AlertTriangle, Layers, ShieldAlert, ShieldCheck, MapPin, CreditCard, CheckCircle2, Truck, Scissors,
} from "lucide-react";
...
export default function CheckoutDrawer({
  ...
  // §3-C — toggle per baris "Jual sebagai sampel"; mode roll eksplisit dilepas saat jadi sampel.
  const toggleSample = (id) => setCart(cart.map((it) => {
    if (it.product.id !== id) return it;
    if (it.is_sample) { const { is_sample, sample_price, sample_unit_price, sample_source, suggested_roll_no, ...rest } = it; return { ...rest, quantity: Math.max(1, Math.round(it.quantity || 1)) }; }
    return { product: it.product, quantity: it.quantity || 1, unit: it.product.base_unit || it.unit, is_sample: true, sample_price: null };
  }));
