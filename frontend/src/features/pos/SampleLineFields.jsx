...
export function SampleLineFields({ item, onUpdateSample }) {
  ...
  return (
    <div data-testid={`sample-fields-${pid}`} className="mt-2 rounded-md border border-[#F3D9A4] bg-[#FFF9EC] p-2 text-[11px]">
      <p className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide text-[#9A5B00]"><Scissors size={11} /> Jual sebagai sampel — permintaan potong ke gudang</p>
      {quote && (
        <p data-testid={`sample-quote-${pid}`} className="mt-1 text-[#3C3C43]">
          {SRC[quote.source] || quote.source}: <b>{formatCurrency(quote.price_per_unit)}</b>/{unit}
